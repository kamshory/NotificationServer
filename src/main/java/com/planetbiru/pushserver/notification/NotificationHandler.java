...
import com.planetbiru.pushserver.constant.ResponseCode;
...
public class NotificationHandler extends Thread
{
	 ...
	 */
	public NotificationHandler(Socket socket, long requestID, DataSource dataSource)
	{
		...
		try 
		{
			...
			else
			{
				if(message.length() > 15)
				{
					...
					notification = new Notification(this.dataSource, this.requestID);
	...
	{
		SocketIO socketIO = new SocketIO(this.getSocket());
		Notification notification = new Notification(this.dataSource, this.requestID);
		...
		try 
		{
			...
			if(!rs.isBeforeFirst())
			{
				String slqInsert = query1.newQuery()
						...
						.toString();
				JdbcUtils.closeStatement(stmt);
				stmt = conn.prepareStatement(slqInsert);		
				success = true;
				this.sendQuestion();
			}
			else
	...
	private boolean unregisterDevice(String deviceID) throws NotificationException, SQLException, DatabaseTypeException
	{
		...
		try 
		{
			...
			if(rs.isBeforeFirst())
			{
				...
				JdbcUtils.closeStatement(stmt);
				stmt = conn.prepareStatement(slqDelete);
